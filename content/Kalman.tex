...
\begin{description}
\item[``/vision/estimated\_pose''] Derived poses from the Kinect arrive here.
\item[``/poseupdate''] The converter listens on this topic to receive poses from laser sensors. 
\item[``laserProcessed''] The laser pose is converted to the same frame as the Kinect frame and re-published here for the EKF.
\item[``/odometry/filtered''] The EKF publishes its filtered pose here.
\item[``fused\_pose''] After the filtered data is converted to world frame, it is re-published here as final output.
